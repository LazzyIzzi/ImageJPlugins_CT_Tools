...
import CT_Tools.FFTutils.DFIparams;
import ij.IJ;
import ij.ImagePlus;
...
public class DFI_JTransforms implements ActionListener, PlugInFilter {
	...
	private ImagePlus DoDFIrecon(DialogParams dlp) {
		...
		// called
		DFIparams dfp = ftu.new DFIparams();
		dfp.paddedSinoWidth = paddedImp.getWidth();
		dfp.semiBicubicLUT = ftu.makeSemiBicubicLUT(dfp.paddedSinoWidth, paddedImp.getHeight(), dlp.padFactor);
		...
		for (int slice = 1; slice <= sinoPairs * 2; slice += 2) {
			...
			reconStk.setPixels(ftu.fftRealToFloat(dfiData), slice);
			reconStk.setPixels(ftu.fftImaginaryToFloat(dfiData), slice + 1);
		}
